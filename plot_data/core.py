...
class TextStyle(DessiaObject):
    ...
    :param font_style: 'Arial', 'Verdana', 'Times New Roman', 'Courier New', 'serif' or 'sans-serif'
    :type font_style: str
    :param text_align_x: "left", "right", "center", "start" or "end". More info on https://www.w3schools.com/tags/canvas_textalign.asp
    :type text_align_x: str
    :param text_align_y: "top", "hanging", "middle", "alphabetic", "ideographic" or "bottom". More info on https://www.w3schools.com/tags/canvas_textbaseline.asp
...
class Text(PlotDataObject):
    ...
    :param max_width: Set a maximum length for the text. If the text is longer than max_width, it is split into several lines.
...
class Line2D(PlotDataObject):
    ...
    :param data: [x1, y1, x2, y2] where (x1, y1) and (x2, y2) are two points that belong to the line.
    :type data: [float, float, float, float]
    :param edge_style: for customization
    :type edge_style: TextStyle
    """
...
class LineSegment2D(PlotDataObject):
    ...
    :param data: [x1, y1, x2, y2] where (x1, y1) and (x2, y2) are two points that belong to the line segment.
    :type data: [float, float, float, float]
    :param edge_style: for customization
...
class Axis(PlotDataObject):
    ...
    :param nb_points_x: the average number of points displayed on the x-axis.
    :type nb_points_x: int
    :param nb_points_y: the average number of points displayed on the y-axis.
    ...
    :param arrow_on: True if you want an arrow to be displayed on axis, False otherwise.
    :type arrow_on: bool
...
class Tooltip(PlotDataObject):
    ...
    :param to_disp_attribute_names: a list containing the attributes you want to display. Attributes must be taken from Dataset's or Scatter's elements.
    ...
    :type text_style: TextStyle
    :param tooltip_radius: a tooltip is rounded-rectangle-shaped. This parameter defines its corners radius.
...
class Dataset(PlotDataObject):
    ...
    :type point_style: PointStyle
    :param tooltip: an object containing all information for drawing tooltips
    :type tooltip: Tooltip
    :param display_step: a value that limits the number of points displayed.
    :type display_step: int
    :param to_disp_attribute_names: [attribute_x, attribute_y] where attribute_x is the attribute displayed on x-axis and attribute_y is the attribute displayed on y-axis.
...
class Arc2D(PlotDataObject):
    """
    A class for drawing arcs. Arc2D is a primitive and can be instantiated by PrimitiveGroup.
    ...
    :param data: a list of relevant points for drawing an arc using BSPline method. This argument is useless unless the arc2D is part of a Contour2D. In such case, the arc must be instantiated by volmdlr.
    :type data: List[dict]
    :param anticlockwise: True if you want the arc the be drawn anticlockwise, False otherwise
...
class Contour2D(PlotDataObject):
    """
    A Contour2D is a closed polygon that is formed by multiple primitives. Contour2D
    can be instantiated by PrimitiveGroup.

    :param plot_data_primitives: a list of primitives (Arc2D, LineSegment2D)
...
class Label(PlotDataObject):
    ...
    :type text_style: TextStyle
    :param rectangle_surface_style: the label's rectangle interior customization
    :type rectangle_syrface_style: SurfaceStyle
...
class PrimitiveGroup(PlotDataObject):
    ...
    :param primitives: a list of Contour2D, Arc2D, LineSegment2D, Circle2D or Line2D
    :type primitives: List[Union[Contour2D, Arc2D, LineSegment2D, Circle2D, Line2D]]
    """
...
class ParallelPlot(PlotDataObject):
    ...
    :type edge_style: EdgeStyle
    :param disposition: either 'vertical' or 'horizontal' depending on how you want the initial disposition to be.
...
def plot_canvas(plot_data_object: Subclass[PlotDataObject],
    ...
    :param plot_data_object: a PlotDataObject(ie Scatter, ParallelPlot, MultiplePlots, Graph2D, PrimitiveGroup or PrimitiveGroupContainer)
    :type plot_data_object: Subclass[PlotDataObject]
    :param debug_mode: uses local library if True, uses typescript library from cdn if False
...
def get_csv_vectors(filepath):
    """
    :param filepath: the csv file's relative path, starting from the script's path.
    :type filepath: str

    :return: a list of vectors (ie a list of dictionaries) that can be set to multiple_plots' or parallelplot's elements for example.
    :rtype: List[dict]
    """
...
def bounding_box(plot_datas: Subclass[PlotDataObject]):
    ...
    :param plot_datas: The target object the bounding_box method has to be called from
