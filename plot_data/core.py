...
class Arc2D(PlotDataObject):
    def __init__(self, cx: float, cy: float, r: float, start_angle: float,
                 end_angle: float, data=None, anticlockwise: bool = None, # data argument is generated by volmdlr Arc2D's plot_data function
                 edge_style: EdgeStyle = None, name: str = ''):           # It is only used if Arc2D is inside a Contour2D
        # anticlockwise's default value is True                       # thus, if an Arc2D is inside a Contour2D, it has to be instantiated with volmdlr
        self.cx = cx                                                  # otherwise, use cx, cy, r, start_angle and end_angle to draw an Arc2D
        self.cy = cy
        self.r = r
...
class PrimitiveGroupsContainer(PlotDataObject):
    def __init__(self, primitive_groups: List[PrimitiveGroup], sizes: List[Tuple[float, float]] = None,
                 ...
                 axis_names: List[str] = None, name: str = ''):  # axis_names=None if the contained primitive_groups don't have to be
