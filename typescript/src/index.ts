export class PlotData {
  ...
  draw_initial() {
    this.init_scale = Math.min(
      this.width / (this.coeff_pixel * this.maxX - this.coeff_pixel * this.minX),
      this.height / (this.coeff_pixel * this.maxY - this.coeff_pixel * this.minY),
    );
    ...
    this.last_mouse1X =
      (this.width / 2 - ((this.coeff_pixel * this.maxX - this.coeff_pixel * this.minX) * this.scaleX) / 2) /
        this.scaleX -
      this.coeff_pixel * this.minX;
    this.last_mouse1Y =
      (this.height / 2 - ((this.coeff_pixel * this.maxY - this.coeff_pixel * this.minY) * this.scaleY) / 2) /
        this.scaleY -
      this.coeff_pixel * this.minY;
      ...
      if (hidden) {
        ...
        if (d.plot_data_states[show_state].hatching != null) {
          this.context.fillStyle = this.context.createPattern(
            d.plot_data_states[show_state].hatching.canvas_hatching,
            'repeat',
          );
        }
  ...
  draw_point(hidden, show_state, mvx, mvy, scaleX, scaleY, d) {
    if (d['type'] == 'point') {
      ...
      var is_inside_canvas =
        x + length >= 0 && x - length <= this.width && y + length >= 0 && y - length <= this.height;
  ...
  draw_graph2D(d, hidden, mvx, mvy) {
    if (d['type'] == 'graph2D' && this.graph_to_display[d.id] === true) {
      ...
      [this.index_first_in, this.nb_points_in, this.index_last_in] = this.get_points_inside_canvas(
        d.point_list,
        mvx,
        mvy,
      );
  ...
  mouse_interaction() {
    ...
    canvas.addEventListener('mousemove', (e) => {
      if (isDrawing === true && !(this.zw_bool || this.select_bool)) {
        ...
        this.draw(
          false,
          0,
          this.last_mouse1X + mouse2X / this.scaleX - mouse1X / this.scaleX,
          this.last_mouse1Y + mouse2Y / this.scaleY - mouse1Y / this.scaleY,
          this.scaleX,
          this.scaleY,
        );
        this.draw(
    ...
    canvas.addEventListener('mouseup', (e) => {
      ...
      var click_on_plus = Shape.Is_in_rect(
        mouse1X,
        mouse1Y,
        this.zoom_rect_x,
        this.zoom_rect_y,
      ...
      var click_on_minus = Shape.Is_in_rect(
        mouse1X,
        mouse1Y,
        this.zoom_rect_x,
        this.zoom_rect_y + this.zoom_rect_h,
        this.zoom_rect_w,
        this.zoom_rect_h,
      );
      var click_on_zoom_window = Shape.Is_in_rect(mouse1X, mouse1Y, this.zw_x, this.zw_y, this.zw_w, this.zw_h);
      var click_on_reset = Shape.Is_in_rect(
        mouse1X,
        mouse1Y,
        this.reset_rect_x,
        this.reset_rect_y,
      ...
      var click_on_select = Shape.Is_in_rect(
        mouse1X,
        mouse1Y,
        this.select_x,
        this.select_y,
      ...
      for (var i = 0; i < this.nb_graph; i++) {
        var click_on_graph_i = Shape.Is_in_rect(
          mouse1X,
          mouse1Y,
          this.graph1_button_x + i * (this.graph1_button_w + this.graph_text_spacing),
          this.graph1_button_y,
          this.graph1_button_w,
          this.graph1_button_h,
        );
        click_on_graph = click_on_graph || click_on_graph_i;
      }
      var click_on_button =
        click_on_plus || click_on_minus || click_on_zoom_window || click_on_reset || click_on_select || click_on_graph;
      ...
      if (mouse_moving) {
        if (this.zw_bool && is_rect_big_enough) {
          ...
          for (var i = 0; i < this.plot_datas.length; i++) {
            ...
            var in_rect = Shape.Is_in_rect(
              this.scaleX * (1000 * d.cx + this.last_mouse1X),
              this.scaleY * (1000 * d.cy + this.last_mouse1Y),
              Math.min(mouse1X, mouse2X),
              Math.min(mouse1Y, mouse2Y),
              Math.abs(mouse2X - mouse1X),
              Math.abs(mouse2Y - mouse1Y),
            );
        ...
        if (this.tooltip_ON) {
          if (
            this.is_include(click_plot_data, this.tooltip_list) &&
            !this.is_include(click_plot_data, this.select_on_click)
          ) {
  ...
  get_points_inside_canvas(list_points, mvx, mvy) {
    //Sous hypothèse que la liste est ordonnée par ordre croissant des x
    var bool = true;
    var k = 0;
...
export class PlotContour extends PlotData {
  plot_datas: any;
  constructor(public data: any, public width: number, public height: number, public coeff_pixel: number) {
    super();
    this.plot_datas = [];
...
export class PlotScatter extends PlotData {
  ...
  draw(hidden, show_state, mvx, mvy, scaleX, scaleY) {
    ...
    this.graph_buttons(
      this.graph1_button_x,
      this.graph1_button_y,
      this.graph1_button_w,
      this.graph1_button_h,
      '10px Arial',
      this.graph_text_spacing,
    );
  }
}
...
export class PlotDataContour2D {
  ...
  constructor(
    public plot_data_primitives: any,
...
export class PlotDataLine2D {
  ...
  constructor(public data: any, public plot_data_states: any, public type: string, public name: string) {
...
export class PlotDataCircle2D {
  ...
  draw(context, first_elem, mvx, mvy, scaleX, scaleY) {
    context.arc(
      scaleX * (1000 * this.cx + mvx),
      scaleY * (1000 * this.cy + mvy),
      scaleX * 1000 * this.r,
      0,
      2 * Math.PI,
    );
...
export class PlotDataPoint2D {
  ...
  draw(context, context_hidden, mvx, mvy, scaleX, scaleY) {
    ...
    if (shape == 'circle') {
      ...
      context.rect(
        scaleX * (1000 * this.cx + mvx) - 1000 * this.size,
        scaleY * (1000 * this.cy + mvy) - 1000 * this.size,
        1000 * this.size * 2,
        1000 * this.size * 2,
      );
      context.stroke();
...
export class PlotDataScatter {
  ...
  public static deserialize(serialized) {
    ...
    return new PlotDataScatter(
      serialized['nb_points_x'],
  ...
  draw_graduations(
    ...
    while (grad_beg_x + i * x_step < grad_end_x) {
      if (
        scaleX * (1000 * (grad_beg_x + i * x_step) + mvx) > axis_x_start &&
        scaleX * (1000 * (grad_beg_x + i * x_step) + mvx) < axis_x_end
      ) {
        if (this.grid_on === true) {
          context.strokeStyle = 'lightgrey';
          Shape.drawLine(
            context,
            [scaleX * (1000 * (grad_beg_x + i * x_step) + mvx), axis_y_start],
            [scaleX * (1000 * (grad_beg_x + i * x_step) + mvx), axis_y_end + 3],
          );
        } else {
          Shape.drawLine(
            context,
        ...
        context.fillText(
          MyMath.round(grad_beg_x + i * x_step, x_nb_digits),
          scaleX * (1000 * (grad_beg_x + i * x_step) + mvx),
          axis_y_end + font_size,
        );
      }
      i++;
    }
    ...
    while (grad_beg_y + (i - 1) * y_step < grad_end_y) {
      if (
        scaleY * (-1000 * (grad_beg_y + i * y_step) + mvy) > axis_y_start &&
        scaleY * (-1000 * (grad_beg_y + i * y_step) + mvy) < axis_y_end
      ) {
        if (this.grid_on === true) {
          context.strokeStyle = 'lightgrey';
          Shape.drawLine(
            context,
            [axis_x_start - 3, scaleY * (-1000 * (grad_beg_y + i * y_step) + mvy)],
            [axis_x_end, scaleY * (-1000 * (grad_beg_y + i * y_step) + mvy)],
          );
        } else {
          Shape.drawLine(
            context,
        ...
        context.fillText(
          MyMath.round(grad_beg_y + i * y_step, y_nb_digits),
          axis_x_start - 5,
          scaleY * (-1000 * (grad_beg_y + i * y_step) + mvy),
        );
      }
      i++;
    }
...
export class PlotDataTooltip {
  ...
  public static deserialize(serialized) {
    ...
    return new PlotDataTooltip(
      serialized['colorfill'],
      serialized['font'],
      serialized['tp_width'],
...
export class PlotDataGraph2D {
  id: number = 0;
  constructor(
    public point_list: PlotDataPoint2D[],
...
export class PlotDataState {
  constructor(
    public color_surface: ColorSurfaceSet,
...
export class ColorSurfaceSet {
  constructor(public name: string, public color: any) {}
...
export class HatchingSet {
  ...
  constructor(public name: string, public stroke_width: number, public hatch_spacing: number) {
